...
if __name__ == "__main__":
    pgn_file_path = "./testdata/1500-1600.pgn"
    ...
    print(f"Combined Prediction Accuracy: {accuracy}")
    ...
    for i in range(min(num_moves_to_display, len(y_test_classes))):
        ...
        try:
            board.push(board.parse_san(test_san_moves[i]))
            legal_moves = [board.san(m) for m in board.legal_moves]
        except ValueError as e:
            print(f"Skipping move due to error: {e}")
            continue
        ...
        print(f"合法手リスト: {', '.join(legal_moves)}")
